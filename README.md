# FinGuard AI — Enterprise Fraud Detection & AML Workflow Platform

*AI-powered financial workflow system (MERN + ML)*
...
In addition to the Python/ML pipeline, this repository now includes a **MERN-based AI-powered financial workflow backend** (`mern-backend/`) built with **Node.js, Express.js, TypeScript, MongoDB, OpenAI API, and Docker**. This backend simulates internal transaction workflows, microservice-style APIs, and AI-driven risk analysis suitable for portfolio or interview demonstrations.
...
### MERN Financial Workflow Backend (Node.js/Express/MongoDB)

The `mern-backend/` folder contains a lightweight microservice-style backend that mirrors how internal financial platforms orchestrate transaction workflows and AI-based risk analysis:

- **Technologies**: Node.js, Express.js, TypeScript, MongoDB (Mongoose), OpenAI API, Docker
- **Microservice-style design**:
  - `src/routes/transactions.ts` – REST APIs for creating and listing transactions
  - `src/routes/workflows.ts` – REST APIs for starting workflows and summarizing runs
  - `src/models/Transaction.ts` – Indexed MongoDB schema for high-volume transaction data
  - `src/models/WorkflowRun.ts` – Workflow execution tracking
  - `src/services/riskService.ts` – AI-based anomaly and risk analysis via OpenAI (with safe heuristic fallback)
- **AI-driven risk analysis**:
  - Calls OpenAI to analyze each transaction and return `riskScore`, `riskLevel`, and anomaly reasons
  - Automatically flags high/critical risk transactions and stores results in MongoDB
- **Scalable REST APIs**:
  - `POST /api/transactions` – Create and score a transaction in one call
  - `GET /api/transactions` – Paginated listing with status filters
  - `POST /api/workflows` – Start a new simulated workflow run
  - `GET /api/workflows/:id/summary` – Aggregated metrics for a workflow (flagged/high-risk counts)

This backend is intentionally focused and minimal, making it easy to showcase:

- Microservice-style API architecture in Node/Express
- MongoDB schema design with indexing for high-volume transaction data
- Integration of AI services (OpenAI) into real-time financial workflows
...
```bash
# Clone repository
git clone https://github.com/deepakgoudasirsi/FinGuard-AI.git
cd FinGuard-AI
...
- GitHub: `@deepakgoudasirsi`
- LinkedIn: https://www.linkedin.com/in/deepakgoudasirsi
